    ...
    return (
        <section className="main">
            ...
            <div className="px-5 md:px-12 flex flex-col justify-center items-center py-4 md:py-8">
                ...
                <div
                    className="my-8 gap-5 md:gap-10 w-full grid grid-cols-5 md:grid-cols-16"
                >
                    {selectedOption === "movie" &&
                        movie
                            ...
                            .map((item, index) => (
                                <Link
                                    to={`${item.provider_id}-${item.provider_name.split(" ").join("_")}/movie?region=${currentRegion.iso_3166_1 ?? region}`}
                                    key={index}
                    ...
                    {selectedOption === "tv" &&
                        tv
                            ?.filter((item) =>
                                item.provider_name.toLowerCase().includes(query)
                            )
                            .map((item, index) => (
                                <Link
                                    to={`${item.provider_id}-${item.provider_name.split(" ").join("_")}/tv?region=${currentRegion.iso_3166_1 ?? region}`}
                                    key={index}
                                    className="w-16 h-16 bg-zinc-600 rounded-md overflow-hidden hover:scale-105 transition-all duration-150 ease-in"
                                >
                                    <img
